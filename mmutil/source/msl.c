...
u16 MSL_AddSampleC( Sample* samp )
{
	...
	while( ftell( F_SAMP ) < fsize )
	{
		...
		// Correctly calculate what Write_SampleData actually wrote to the header
		u32 write_len1, write_len2;
		if( target_system == SYSTEM_NDS ) {
			int div = (samp->format & SAMPF_16BIT) ? 2 : 4;
			if( samp->loop_type ) {
				write_len1 = samp->loop_start / div;
				write_len2 = (samp->loop_end - samp->loop_start) / div;
			} else {
				write_len1 = 0;
				write_len2 = samp->sample_length / div;
			}
		} else {
			write_len1 = samp->sample_length;
			write_len2 = samp->loop_type ? (samp->loop_end - samp->loop_start) : 0xFFFFFFFF;
		}

		samp_match = true;
		if( write_len1 == samp_len && write_len2 == samp_llen && sformat == target_sformat )
		{
			// verify sample data (Fixed to loop to samp->sample_length)
			...
			{
				// Fixed to skip by bytes instead of arrays
				int bytes_read = (st + 1) * ((samp->format & SAMPF_16BIT) ? 2 : 1);
				skip8f( (h_filesize - SAMPLE_HEADER_SIZE) - bytes_read, F_SAMP );
			}
		}
		else
		{
			skip8f( h_filesize - SAMPLE_HEADER_SIZE, F_SAMP ); 
		}
		samp_id++;
	}
	fclose( F_SAMP );
